...
@app.get("/shops_employees/", response_model=list[schemas.AllInfo])
...
@app.delete("/shop_employees/{shop_employees}")
def delete_shop(id_shop_employees: int, db: Session = Depends(get_db)):
    db_shop = crud.get_shops_employee_id(db, shop_employees_id=id_shop_employees)
    if db_shop is None:
        raise HTTPException(status_code=400, detail="There is no store with this id")
    crud.delete_shops_employee(db, id_shop_employees=id_shop_employees)
    return HTTPException(status_code=200, detail="The store has been deleted")
